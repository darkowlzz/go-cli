...
func newInspectCommand(storageosCli *command.StorageOSCli) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "inspect [OPTIONS] [USER...]",
		Short: "Display detailed information on one or more user(s) - if none are specified all users are inspected",
		Args:  nil,
...
func runInspectAll(storageosCli *command.StorageOSCli, opt inspectOptions) error {
	...
	getListFunc := func() ([]interface{}, []byte, error) {
		...
		intfs := make([]interface{}, len(users))
		for i, u := range users {
			intfs[i] = u
		}
