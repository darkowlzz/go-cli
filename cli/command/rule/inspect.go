...
func newInspectCommand(storageosCli *command.StorageOSCli) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "inspect [OPTIONS] [RULE...]",
		Short: "Display detailed information on one or more rules - if none are specified all rules are inspected",
		Args:  nil,
...
func runInspectAll(storageosCli *command.StorageOSCli, opt inspectOptions) error {
	...
	getListFunc := func() ([]interface{}, []byte, error) {
		...
		intfs := make([]interface{}, len(rules))
		for i, r := range rules {
			intfs[i] = r
		}
